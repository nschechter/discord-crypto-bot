	...
	switch (args[1].toLowerCase()) {
		...
		case "all":
			...
			if (customName && customName === 'jsonpath' || 'xpath') {
				message.channel.send(customName === 'jsonpath' ? 
					bot.getCustomApisHandler().getCustoms().reduce((arr, custom) => arr.concat(custom.dataPoints), []).map((dataPoint) => {
						return `${dataPoint.name} - ${dataPoint.activated} - ${dataPoint.selector} - ${dataPoint.value}`
					}) : bot.getCustomScrapersHandler().getCustoms().reduce((arr, custom) => arr.concat(custom.dataPoints), []).map((dataPoint) => {
						return `${dataPoint.name} - ${dataPoint.activated} - ${dataPoint.selector} - ${dataPoint.value}`
					}));
