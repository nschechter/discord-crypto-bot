	...
	switch (args[1].toLowerCase()) {
		...
		case "coin":
			...
			Object.keys(coins).forEach((coinName) => {
				message.channel.send(`Reporting ${coinName}: ${coins[coinName][Object.keys(coins[coinName])[0]]}`);
			});
		break;
		case "api":
			bot.getCustomApisHandler().addDataPoint(args[2], args[3], args[4]).then((dataPoint) => {
