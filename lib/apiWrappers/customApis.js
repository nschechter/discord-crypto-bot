		...
		.reduce((obj, dataPoint) => {
		 if (Object.keys(obj).indexOf(dataPoint.coin) !== -1) {
		 	...
		 	if (dataPoint.name.includes('BTC-price')) {
		 		let btcPrice = parseFloat(this.getDataPoints().find((dataPoint) => dataPoint.name === 'BTC-USD-price'));
		 		debugger;

		 		// creating usd price for every coin paired with BTC or ETH
		 		obj[dataPoint.coin]['USD-price'] = dataPoint.value * btcPrice;
		 	} else if (dataPoint.name.includes('ETH-price')) {
		 		let ethPrice = parseFloat(this.getDataPoints().find((dataPoint) => dataPoint.name === 'ETH-USD-price'));
		 		obj[dataPoint.coin]['USD-price'] = dataPoint.value * ethPrice;
		 	}
		 }
		 return obj;
		}, coins);
